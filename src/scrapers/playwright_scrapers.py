...
async def get_browser_context(
    ...
    """
    Creates and returns a browser context.
    Returns: context, browser, playwright
    Note: Stealth is applied per-page in the scraper (not here).
    """

    if use_patchright:
        try:
            from patchright.async_api import async_playwright as pr_async_playwright
            p = await pr_async_playwright().start()
            browser_type = p.chromium
            logger.info("[Browser] Using Patchright")
        except ImportError:
            logger.warning("[Browser] Patchright not found, falling back to Playwright")
            from playwright.async_api import async_playwright as pw_async_playwright
            p = await pw_async_playwright().start()
            browser_type = p.chromium
    else:
        from playwright.async_api import async_playwright as pw_async_playwright
        p = await pw_async_playwright().start()
    ...
    try:
        ...
        if "chrome" in str(e).lower():
            logger.warning("[Browser] Google Chrome channel not found, falling back to default Chromium")
            browser = await browser_type.launch(
                headless=headless,
                args=launch_args,
                proxy=proxy,
...
class GenericPlaywrightScraper:
    ...
    async def search(self, query: str) -> List[ProductPrice]:
        ...
        products = []
        try:
            # Use the improved get_browser_context
            context, browser, playwright_instance = await get_browser_context(headless=True)
            ...
            for pattern in search_patterns:
                ...
                try:
                    ...
                    if "avivdrinks.co.il" in search_url:
            ...
            # ── Cleanup of context, browser and playwright ──────────────────
