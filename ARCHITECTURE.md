...
3. **Hard Store Enhanced Retries & Dedicated Circuit Breaker** (in `html_scrapers.py` only): The three hard stores (**paneco.co.il**, **the-importer.co.il**, **mashkaot.co.il**) use `HARD_STORES` config + `HardStoreCircuitBreaker` singleton (dedicated, in-memory, separate from general breaker). Features: 5 attempts, base_delay=8s, backoff_factor=1.8, threshold=2. Per-fetch: check if OPEN (short-circuit), log attempt+failure_reason+circuit_state (CLOSED/HALF-OPEN/OPEN/recovered), record_result on success/failure. Extensible: add domain to HARD_STORES. Focused exclusively on these three stores.
